...
def remove_from_favorites(fund_id):
    ...
    if favorite_entry:
        before_count = AddFundToFavorites.query.filter_by(user_id=user.id).count()
        db.session.delete(favorite_entry)
        db.session.commit()
        after_count = AddFundToFavorites.query.filter_by(user_id=user.id).count()
...
def fund_details(cik):
    ...
    # Convert SQLAlchemy to pandas DataFrame
    ...
    # Sort by Company Name and Accession Number to ensure order for comparison
    ...
    if most_recent_accession:
        ...
        if previous_accession:
            ...
            try:
                ...
                # Add Change Amount and Change Percentage
        ...
        merged_holdings_df['Previous Share Amount'] = merged_holdings_df['Previous Share Amount'].fillna(0).astype(
            int)
        merged_holdings_df['Change Amount'] = ((merged_holdings_df['Share Amount'] -
                                                merged_holdings_df['Previous Share Amount'])).astype(int)
...
def admin_dashboard():
    ...
    if not hasattr(current_user, 'is_admin') or not current_user.is_admin:
        flash('Access denied. You need to be an admin to view this page.', 'error')
        # Redirect to admin_login instead of home
        return redirect(url_for('routes.admin_login'))
